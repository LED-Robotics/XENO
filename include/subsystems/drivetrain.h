...
public:
    DrivetrainSubsystem(const std::initializer_list<int8_t> &left11_w,
                        const std::initializer_list<int8_t> &right11_w,
                        pros::Imu imu, pros::Rotation odom)
          ...
          odom(std::move(odom))
    {
        ...
        imu.reset(pros::competition::is_autonomous() ||
                  pros::competition::is_disabled() || AUTON == SKILLS);
    }
    ...
    void setDriveSpeeds(DriveSpeeds lastSpeeds, DriveSpeeds nextDriveSpeeds = {0.0, 0.0})
    {
        ...
        const double uLinear =
            (CONFIG::DRIVETRAIN_LINEAR_VELOCITY_FF)*Eigen::Vector2d(nextDriveSpeeds.linearVelocity.getValue(), linearAcceleration.getValue());
        const double uAngular =
            (CONFIG::DRIVETRAIN_ANGULAR_VELOCITY_FF)*Eigen::Vector2d(nextDriveSpeeds.angularVelocity.getValue(), angularAcceleration.getValue());
                            ...
                            const bool flip)
    {
        ...
        return new InstantCommand(
            [this, mean, covariance, flip, angle]()
            { this->initNorm(mean, covariance, angle, flip); }, {this});
    }
    ...
    {
        return new RunCommand([this, left, right]()
                              { this->setPct(left, right); }, {this});
    }
