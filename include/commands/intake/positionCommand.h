...
public:
    TopIntakePositionCommand(TopIntakeSubsystem *intake, float tolerance, const std::function<float(float)> &position_callback,
                             const PID &pid) : intake(intake), pid(pid), tolerance(tolerance), positionCallback(position_callback)
    {
                               ...
                               PID pid = CONFIG::TOP_INTAKE_PID)
    {
        return new TopIntakePositionCommand(
            intake, tolerance,
            [setpoint](float position)
            { return static_cast<float>(std::floor(position) + setpoint); }, pid);
    }
                             ...
                             PID pid = CONFIG::TOP_INTAKE_PID)
    {
        return new TopIntakePositionCommand(
            intake, tolerance,
            [setpoint](float position)
            { return static_cast<float>(std::round(position) + setpoint); }, pid);
    }
